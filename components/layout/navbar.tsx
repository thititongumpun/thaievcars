  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border bg-card/95 backdrop-blur">
      ...
      <nav className="flex gap-1 overflow-x-auto border-t border-border px-4 py-2 md:hidden">
        {items.map((item) => (
          <Link key={item.href} href={item.href} className="whitespace-nowrap rounded-md px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-muted hover:text-foreground">
            {item.label}
          </Link>
        ))}
      </nav>
